...
export const Active = ({ navigation }) => {
            ...
            () => {
                axios.get('http://18.207.182.108:8085/user/getReferredUsers/zlJp6nhMZ7ZgYRa7r8czJKNfQEKuKMkk3t', {
                    ...
                    .then(function (response) {
                        ...
                        setTreedata(response.data.referredUsers)
    ...
    <DataTable style={{backgroundColor:'white',borderColor:'black',borderWidth:2,borderRadius:20}}>
       <DataTable.Header style={{borderBottomColor:'black',borderBottomWidth:1}}>
        ...
        {treedata.map(item =><DataTable.Row>
         <DataTable.Cell>{moment(parseInt(item.date)).format("DD/MM/YYYY")}</DataTable.Cell>
         <DataTable.Cell numeric>{item.email}</DataTable.Cell>
         <DataTable.Cell numeric>{item.name}</DataTable.Cell>
         <DataTable.Cell numeric>{item.totalEarning}</DataTable.Cell>
       </DataTable.Row>)}
